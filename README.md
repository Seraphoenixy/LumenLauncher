...
LumenLauncher 是一个基于 .NET 10 和 WPF 构建的本地 Windows 启动器。它将常用应用、便携程序、配置目录中的文件夹和网页 Quicklink 汇集到一个搜索框中，支持键盘快速启动或打开。内置拼音索引，可使用全拼或首字母搜索中文应用名。

## 功能

- 搜索并启动开始菜单、桌面、`App Paths`、`PATH` 和 Windows 内置应用。
- 发现并启动 Microsoft Store / MSIX 应用。
- 扫描配置目录下的便携 `.exe` 程序，自动过滤卸载程序、更新器等低质量候选。
- 索引配置目录下最多三层的文件夹，并在资源管理器中打开。
- 配置静态或带查询参数的网页 Quicklink。
- 为所有应用和文件夹名称生成全拼和首字母拼音索引，支持拼音搜索（如输入 `jsq` 匹配"计算器"）。
- 记录使用次数和最近打开结果；搜索框为空时展示最近使用记录。
- 支持内置命令：输入 `rebuild index` 重建索引，输入 `exit lumen` 退出程序。
- 支持全局快捷键（默认 `Alt+W`）、托盘菜单、开机自启动、窗口位置保存和失焦自动隐藏。
- 使用 SQLite 保存本地索引；重建时会同步移除已卸载、已删除或不再位于索引目录中的项目；不上传索引或使用数据。

## 使用

启动后输入名称或拼音搜索，使用 `↑` / `↓` 选择结果，按 `Enter` 执行，按 `Esc` 隐藏窗口。搜索框为空时展示最近使用的结果。默认全局快捷键 `Alt+W` 可显示或隐藏启动器，也可在设置中修改。左键单击托盘图标也可切换显示。
...
Lumen 仅索引文件夹，默认最多递归三层；保存配置后会在后台刷新索引。为控制索引规模和避免循环，`.git`、`.svn`、`node_modules`、`bin`、`obj`、`$RECYCLE.BIN`、`System Volume Information`、隐藏/系统目录及链接目录会被跳过。
...
对于带 `{query}` 的 Quicklink，输入 `g Lumen Launcher` 即可用 Google 搜索该文本。Quicklink 目前仅接受 `http` 或 `https` 地址；未指定协议时自动补 `https://`。
...
GitHub Actions 会在推送形如 `v0.1.0` 的标签时运行测试，构建 `win-x64` 与 `win-arm64` 自包含产物，并创建草稿 GitHub Release 上传 ZIP 安装包。也可通过 Actions 页面手动触发工作流。
...
- `src/Lumen.Indexer`：应用和文件夹名称的拼音索引器，在索引重建后自动运行。
