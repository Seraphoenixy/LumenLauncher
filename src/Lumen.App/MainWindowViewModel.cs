...
public sealed record SearchResultViewModel(SearchResult Result, System.Windows.Media.ImageSource? Icon)
{
 ...
 public string TypeLabel=>Result.Type switch { SearchResultType.Application=>"应用",SearchResultType.PortableApplication=>"便携应用",SearchResultType.Command=>"命令",_=>"结果"};
 public string IconGlyph=>Result.Type switch { SearchResultType.Application=>"◫",SearchResultType.PortableApplication=>"◈",SearchResultType.Command=>"›",_=>"·"};
}
public partial class MainWindowViewModel : ObservableObject
{
 ...
 private async Task SearchAsync(string text){var mine=Interlocked.Increment(ref _version);_searchCts?.Cancel();var cts=_searchCts=new CancellationTokenSource();try{await Task.Delay(75,cts.Token);var data=await _search.SearchAsync(new(text),cts.Token);if(mine!=_version)return;await System.Windows.Application.Current.Dispatcher.InvokeAsync(()=>{if(mine!=_version)return;Results.Clear();_icons.Reset();foreach(var r in data)Results.Add(new(r,_icons.GetIcon(r.IconKey)));SelectedResult=Results.FirstOrDefault();});}catch(OperationCanceledException){}}
 public void ClearTransientResults(){Interlocked.Increment(ref _version);_searchCts?.Cancel();Results.Clear();SelectedResult=null;_icons.Reset();}
